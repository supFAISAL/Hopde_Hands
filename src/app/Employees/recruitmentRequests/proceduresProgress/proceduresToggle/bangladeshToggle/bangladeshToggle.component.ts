...
@Component({
  selector: 'app-bangladeshToggle',
  templateUrl: './bangladeshToggle.component.html',
  styleUrls: ['./bangladeshToggle.component.css']
})
export class BangladeshToggleComponent implements OnInit {
  @Input() orderData;
  isVisible = false;
  ...
  submitForm(): void {
    if(this.fingerPrints!==undefined&&this.allProceduresStatus[3].date.length===0){
      setTimeout(() => {
      this.allProceduresStatus[3].date.push(this?.fingerPrints[0])
      this.allProceduresStatus[3].date.push(this?.fingerPrints[1])
    }, 2000); 
    }
